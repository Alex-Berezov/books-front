...
/**
 * Response of `POST /uploads/presign` (`PresignResponseDto` on the backend).
 *
 * Поле адреса называется `url`, а не `uploadUrl`: рукописный тип обещал второе,
 * и загрузка уходила на `undefined` (`LEGACY-372`). Форма сверяется со схемой
 * гейтом `yarn check:type-sync`.
 */
export interface PresignUploadResponse {
  /** Short-lived token scoped to the upload URL (sent as `X-Upload-Token` to `/uploads/direct`). */
  token: string;
  /** Absolute URL to send the binary body to. */
  url: string;
  /** HTTP method the storage driver expects on that URL. */
  method: 'POST' | 'PUT';
  /** Headers the storage driver expects alongside the body. */
  headers?: Record<string, string>;
  /** Token and URL lifetime in seconds. */
  ttlSec: number;
