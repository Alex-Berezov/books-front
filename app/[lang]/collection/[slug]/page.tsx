...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const seo = await resolveSeo(supportedLang, 'collection', slug);
...
export default async function CollectionDetailPage({ params }: Props) {
  ...
  let seoData;
  try {
    seoData = await resolveSeo(supportedLang, 'collection', slug);
  } catch (error) {
    logError('Error loading SEO for collection on server:', error);
  }
