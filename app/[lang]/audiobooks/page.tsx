...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  const countRes = await getBookCards(supportedLang, 1, 1, { type: LANDING_TYPE }).catch(
    (error) => {
      logError('Error counting books for audiobooks metadata:', error);
      return null;
    }
  );
  ...
  const meta = getPageMetadata(supportedLang, '/audiobooks', title, description, currentPage);
...
export default async function AudiobooksPage({ params, searchParams }: Props) {
  ...
  const [booksRes, categoriesRes, genresRes] = await Promise.all([
    ...
    getBookCards(supportedLang, currentPage, PAGE_SIZE, { type: LANDING_TYPE }),
