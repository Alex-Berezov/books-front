...
export default async function GenreDetailPage({ params }: Props) {
  ...
  let seoData;
  try {
    seoData = await resolveSeo(supportedLang, 'genre', slug);
  } catch (error) {
    logError('Error loading SEO for genre on server:', error);
  }
  ...
  return (
    <>
      ...
      <TaxonomyDetailClient lang={supportedLang} slug={slug} taxonomyType="genre" />
    </>
  );
}
