...
export default async function GenreDetailPage({ params, searchParams }: Props) {
  ...
  try {
    ...
    if (isNotFoundError(error)) {
      ...
      if (retired && retired !== slug) {
        permanentRedirect(`/${supportedLang}/genre/${retired}`);
      }
    }
  ...
  const destination = resolveTaxonomyDestination(data?.category, supportedLang, 'genre', slug);
  if (destination) {
  ...
  const itemListJsonLd = buildItemListJsonLd(
    ...
    `${siteUrl}/${supportedLang}/genre/${slug}`
  );
