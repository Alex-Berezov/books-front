...
type Props = {
  params: { lang: string; authorSlug: string };
};
...
export default function AuthorDetailPage({ params }: Props) {
  const { lang, authorSlug } = params;
  ...
  const displayName = authorBooks.length > 0 ? authorBooks[0].author : toTitleCase(searchName);
    ...
    <main className={styles.authorPage}>
      <div className={styles.container}>
        ...
        <section className={styles.booksSection}>
          <h2 className={styles.sectionTitle}>Books by {displayName}</h2>
