...
const PAGE_SIZE = 24;
const LANDING_SORT = 'new' as const;
const TITLE_KEY = 'new';
...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  const countRes = await getBookCards(supportedLang, 1, 1, { sort: LANDING_SORT }).catch(
    (error) => {
      logError('Error counting books for new-releases metadata:', error);
      return null;
    }
  );
  ...
  const meta = getPageMetadata(supportedLang, '/new-releases', title, description, currentPage);
...
export default async function NewReleasesPage({ params, searchParams }: Props) {
  ...
  const routeUrl = `${siteUrl}/${supportedLang}/new-releases`;

  const breadcrumbJsonLd = buildBreadcrumbJsonLd(
    [
      { name: dict.breadcrumb.home, url: `${siteUrl}/${supportedLang}` },
      { name: dict.footer.newReleases, url: routeUrl },
    ],
    routeUrl
  );
