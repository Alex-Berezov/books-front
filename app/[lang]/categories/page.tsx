...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const page = await fetchPageBySystemKey(lang, TAXONOMY_OVERVIEW_CONFIGS.category.pageKey);
...
export default async function CategoriesIndexPage({ params }: Props) {
  ...
  const [page, items] = await Promise.all([
    fetchPageBySystemKey(lang, TAXONOMY_OVERVIEW_CONFIGS.category.pageKey),
