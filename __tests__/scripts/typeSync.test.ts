  ...
  function runGate(args: string[] = [], env: Record<string, string> = {}) {
    try {
      const stdout = execFileSync(
        ...
        {
          encoding: 'utf8',
          // CI гасится намеренно: под `CI=true` гейт игнорирует `--update`, и посев
          // снимка в песочнице молча превращался бы в обычную сверку — спека зеленела
          // бы на машине и краснела в конвейере. Случай «--update под CI не переписывает»
          // ставит CI обратно и проверяется отдельно.
