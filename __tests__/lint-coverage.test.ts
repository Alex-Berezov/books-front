...
describe('строгость правил импорта (LEGACY-152)', () => {
   ...
   * тот же класс, что описан над `testTimeout` в `vitest.config.ts`. С 06.09.2026 порог
   * там 15 секунд, а причину сняли переводом тестов без DOM в окружение `node`
   * (`LEGACY-105`), поэтому нагрузка от этого образца стала весомее прежнего:
   * поднимать порог обратно нельзя, чинить надо стоимость разбора. На `.mjs` override не действует,
