...
describe('localProgress', () => {
   ...
   * Синхронизации в живом времени нет — побеждает последняя запись. Здесь важно
   * и второе: карта переживает такую гонку целиком, а не рвётся.
   *
   * ✅ Этот тест фиксирует **принятое** поведение, а не временное. Локальная
   * закладка хранит последнюю позицию читателя, а не самую дальнюю, — решение
   * арбитра 20.09.2026 (`LEGACY-269`, исход «принято»). Цена названа там же:
   * одна закладка при одновременном чтении одной книги в двух вкладках.
   *
   * Что пробовали и почему не берём:
   * — страж «дальняя глава не откатывается ближней» (пробовался 06.09.2026,
   *   снят до коммита — искать revert в истории бесполезно) — запрещает
   *   и законный возврат читателя к ранней главе;
   * — CAS по `updatedAt` даёт тот же итог «побеждает последний»: отставшая
   *   вкладка перечитает свежую сторону и запишет поверх неё своим более новым
   *   `updatedAt`. Хранилище и так перечитывается перед каждой записью и карта
   *   не кэшируется (`lib/reading-progress/localProgress.ts:186-202`, `:17-23`),
   *   так что сверять нечего;
   * — слушатель `storage` менял бы не хранилище, а экран открытой вкладки, и ему
   *   сегодня некуда сесть: закладка читается ровно один раз за открытие книги,
   *   под защёлкой `hasRestoredProgress` (`ReaderClient.tsx:124-150`,
   *   `ListenClient.tsx:170-198`). Дать ей сработать повторно — это и есть переброс
   *   читателя на чужую главу посреди чтения, прямо противоречащий принятому
   *   «побеждает последняя позиция».
   *
   * Запись переоткроется, если закладка станет отвечать за «дочитано дальше
   * всего» (прогресс-бар, достижения, «продолжить с самого дальнего») либо
   * появится синхронизация вкладок для вошедшего. Менять ожидание этого теста
   * без такого события — значит чинить то, что решено не чинить.
   */
