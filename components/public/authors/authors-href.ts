 ...
 *
 * ⚠️ С 06.09.2026 само правило в `middleware.ts` исправлено (`LEGACY-186`): регистр
 * сверяется **вне** последовательностей `%XX`, и заглавный hex 301 больше не вызывает.
 * Эта функция остаётся единственной точкой сборки адреса не ради обхода редиректа,
 * а ради одной формы записи: без неё `%D0%B4` и `%d0%b4` стали бы двумя адресами
 * одной страницы, которые теперь оба отвечают 200 и разводятся только canonical.
 *
