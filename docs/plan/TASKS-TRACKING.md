...
**ОБЯЗАТЕЛЬНО:** После завершения каждой подзадачи делать **commit + push в GitHub**!
...
5. ✅ Отметить подзадачу здесь как 🟢
6. ✅ `git add docs/plan/TASKS-TRACKING.md && git commit -m "docs: mark M0.4 as completed" && git push`
...
- [ ] 🔴 **M0.4** Переключатели языка
  - [ ] `components/LanguageSwitcher.tsx` для публички
  - [ ] `components/admin/AdminLanguageSwitcher.tsx`
  - [x] `lib/i18n/lang.ts` с утилитами языков (частично создано)
...
**Общий прогресс:** 4/67 задач (6%)

---

## 🎯 Текущий фокус

**Сейчас:** M0.4 — Переключатели языка 🟡  
**Завершено:** M0.1, M0.2, M0.3, M0.7 ✅  
**Следующее:** M0.4 → M0.5 → M0.6 → Завершение M0

---

**Последнее обновление:** 19 октября 2025 - 14:30
