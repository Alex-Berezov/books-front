...
⚠️ **Имя токена не выводится по аналогии.** Шкала размеров у шрифта и межстрочного интервала
названа иначе, чем у отступов: средняя ступень — это `$font-size-base` и `$line-height-base`,
а имён font-size-md и line-height-normal (написаны без знака доллара, потому что их нет
в `styles/tokens.scss`) не существует. Ошибку видно только на сборке, сообщением
«Undefined variable», — ни линт, ни типы её не поймают. Предупреждение перенесено сюда
07.09.2026 из удалённого чеклиста правил (`LEGACY-168`).

Это **выдержка**, а не весь набор: значения выше сверены с `styles/tokens.scss` 07.09.2026 и
совпадают, но за её пределами остаются наведённые варианты семантических цветов, третья ступень
фона и текста и вся брендовая группа публичного сайта (терракота публичного сайта, зелёный
Bibliaris, золото) вместе с их тёмными вариантами. Верстая публичную страницу, бери имена
из `styles/tokens.scss`, а не из этого списка: полный состав — командой
`grep -oE "^.color-[a-z0-9-]+" styles/tokens.scss`.
