...
`tsc`. Красное — регресс маршрута из снимка покрытия `scripts/type-sync/covered-types.json`
(57 маршрутов), пропажа утверждения у покрытого маршрута и **новое** покрытие, не внесённое
в снимок. Законное изменение — `yarn type-sync:snapshot`, результат идёт в тот же коммит;
`--update` при `CI=true` игнорируется. ⚠️ Непокрытых маршрутов 72 из 152 утверждений, и большинство правится **не здесь**:
корень в DTO бэкенда (`LEGACY-374`, строка очереди `Q7`). Подгонять рукописный тип под беднее
схему, гасить утверждение `as`, `@ts-ignore` или сужением типа — запрещено решением арбитра
от 10.09.2026: так теряются верные сведения о полях, которые ручка отдаёт.
